...
export default function BigCheeseDetail() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <button onClick={()=>navigate(-1)} className="text-gray-600 mb-4">← Retour</button>
      <h1 className="text-3xl font-bold mb-4">{product.name}</h1>
      <img src={product.image} alt={product.name} className="w-full rounded mb-6" />
      <p className="text-xl font-semibold mb-6">Prix de base : {product.price} €</p>

      {product.options.map((opt,g) => (
        <section key={g} className="mb-8">
          <h2 className="text-2xl font-semibold">{opt.name}</h2>
          <p className="text-sm text-gray-500 mb-2">Choix max : {opt.max}</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {opt.choices.map((c,j) => (
              <div key={j} className="border rounded p-4 text-center">
                <img src={c.image} alt={c.label} className="h-24 mx-auto mb-2" />
                <p className="font-medium">{c.label}</p>
                {c.price>0 && <p className="text-green-600">+{c.price} €</p>}

                {opt.type==='checkbox' && (
                  <div className="flex items-center justify-center space-x-3 mt-3">
                    <button onClick={()=>handleCheckbox(g,j,-1)} className="px-2 py-1 bg-gray-200 rounded">–</button>
                    <span>{sel[g][j]}</span>
                    <button onClick={()=>handleCheckbox(g,j,1)} className="px-2 py-1 bg-gray-200 rounded">+</button>
                  </div>
                )}
                ...
                {opt.type==='select' && j===0 && (
                  <select
                    value={sel[g]}
                    onChange={e=>setSel(p=>({ ...p, [g]: +e.target.value }))}
      ...
      <button
        onClick={()=>console.log('Panier:', { productId:99, sel })}
